...
#args
init_args = argparse.ArgumentParser(description='creates an n-gram markov bot with very reasonable political opinions indeed')
...
init_args.add_argument('gramsize',help='the size of the grams for the program',
                       type=int)
init_args.add_argument('-o','--output', help='specify the output of the file, must be either .txt (plain text) or .csv (markov table), if non specified, will print plain text to stdio',
                       type=argparse.FileType('r'),
                       default=sys.stdout)
